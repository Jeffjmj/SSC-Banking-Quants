class SiddhiQuizApp {
  ...
  async load() {
    const datasetPaths = [
      '../Siddhi/ssc_math_numeric_qna_clean.json',
      '/Siddhi/ssc_math_numeric_qna_clean.json',
    ...
    for (const path of datasetPaths) {
      try {
        ...
        this.setupTopics();
        this.filterByTopic();
        this.meta.textContent = `Loaded ${data.length} questions from ${path}`;
        return;
      } catch (_) {
    ...
    document.getElementById('q-text').textContent =
      'Could not load quiz data. Run a local server from repo root (python -m http.server 8010) and open /quiz-apps/index.html, or copy ssc_math_numeric_qna_clean.json into quiz-apps/.';
  }
  ...
  render() {
    ...
    document.getElementById('meta').textContent = `Showing ${this.index + 1} / ${this.filtered.length}`;
